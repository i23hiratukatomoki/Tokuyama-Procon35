...
public class ModelUser {
    ...
    private static int using_model2_2(Context context) {
        ...
        */
        ...
        switch (ans) {
            case 0://アリ
                return using_model3_2a(context);
            case 1://ハサミムシ
                return 25;
            case 2://ハチ
                return 26;
    ...
    private static int using_model2_5(Context context) {
        ...
        //推論メソッドを使っている。(resultに結果を格納)
        try {
            // model2_5は4クラスなのでclasses_num=4で推論を実行
            result = runInference(4);
    ...
    private static int using_model3_2a(Context context) {
        try {
            // 使用する推論モデル(model3_2a)の読み込み
            tfLiteModel = new Interpreter(loadModelFile(context, "model3_2a.tflite"));
            System.out.println("Successes:loading model3_2a");
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        //推論メソッドを使っている。(resultに結果を格納)
        try {
            // model3_3bは3クラスなのでclasses_num=3で推論を実行
            result = runInference(3);
            System.out.println("model3_2a 推論成功");
            Log.d("Inference", "Prediction model3_2a: " + Arrays.toString(result));
        } catch (Exception e) {
            e.printStackTrace();
        }
