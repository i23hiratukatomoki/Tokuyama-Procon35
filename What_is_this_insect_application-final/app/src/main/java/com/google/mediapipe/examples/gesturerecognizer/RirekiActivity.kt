...
class RirekiActivity : AppCompatActivity() {
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        menuInflater.inflate(R.menu.menu_rireki, menu) // メニューを読み込む
        return true
    }
    // メニューアイテムが選択されたときの処理
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.setadult -> {
                val isAdult = MainActivity.BooleanManeger.isAdult
                MainActivity.BooleanManeger.isAdult = !isAdult
                if(MainActivity.BooleanManeger.isAdult){
                    Toast.makeText(this, "大人モード", Toast.LENGTH_SHORT).show()
                }else{
                    Toast.makeText(this, "こどもモード", Toast.LENGTH_SHORT).show()
                }
                true
            }
